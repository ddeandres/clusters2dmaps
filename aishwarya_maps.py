...
selecth = np.load('/home2/deandres/clusters2dmaps/aishwarya.npy')
#HID M200 Rid
save_path = '/data7/users/deandres/Aishwarya_maps/'
...
reso = 0.08333333333333334 # in arcmin

regions = np.int64(selecth[:,2])
    ...
    for j, s, hid in zip(idr, sn , Hids): # loop over snaps
        ...
        for RA in RAs:
            ...
            for particle in particles:
                ...
                # Rotations
                #------------
                mock_redshift = head.Redshift
                if mock_redshift<=0.05:
                    mock_redshift= 0.1 + head.Redshift
                    
                
                
                rot = rotate_data(pos_inside,RA)[0]
                mask = np.where((rot[:,0]<=2*rr)&(rot[:,0]>=-2*rr)&
                        (rot[:,1]<=2*rr)&(rot[:,1]>=-2*rr)&
                        (rot[:,2]<=2*rr)&(rot[:,2]>=-2*rr))
                rot = rot[mask]
                ...
                #------------
                reso_kpc_comov = (cosmo.kpc_comoving_per_arcmin(mock_redshift).value)*reso
                
                
                N = int(4*rr/0.678/reso_kpc_comov)
                x = rot[:, 0] 
                y = rot[:,1] 
                ...
                pixel_area = reso_kpc_comov**2
                
                #reso_kpc_proper = (cosmo.kpc_proper_per_arcmin(mock_redshift).value)*reso
                img = img/pixel_area
            ...
            write_fits_image_totalmass(img_tot,
                             ...
                             comments=("Simulation Region: " + clnum,
                                      ...
                                      "Simulation redshift: " + str(head.Redshift)[:7],
                                       "mock redshift:"+ str(mock_redshift)[:7],
                                      "log M_200 = "+str(np.log10(halo[idg[0],3]))[:8]+" Msun/h",
                                      "R_200 = "+str(rr)[:8]+" kpc/h",
                                        " pixel resolution = "+ str(pixel_area)[:8]+ "kpc**2",
                                       "IMPORTANT: ALL DISTANCES ARE COMOVING 1/(1+mock_redshift) to translate into proper"
                                      )
                                      )
